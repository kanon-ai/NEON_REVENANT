...
pipe.listen('\\\\.\\pipe\\'+name,()=>{
 child=spawn(process.env.OPENMSX_EXE||'C:\\Program Files\\openMSX\\openmsx.exe',['-control','pipe:'+name,'-machine','Panasonic_FS-A1ST',...process.argv.slice(2)],{windowsHide:true,stdio:['ignore','pipe','pipe']});
...
const server=httpServer(async(req,res)=>{if(req.method!=='POST'){res.writeHead(405);res.end();return;}let b='';for await(const c of req)b+=c;try{let v=await command(b);res.writeHead(200,{'Content-Type':'text/plain; charset=utf-8'});res.end(v);}catch(e){res.writeHead(500);res.end(String(e));}}).listen(18791,'127.0.0.1');
