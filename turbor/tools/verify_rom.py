...
results=[]
def cmd(s):
    r=urllib.request.Request('http://127.0.0.1:18791',data=s.encode(),method='POST')
...
check('SCREEN4-mode',int(cmd('debug read {VDP regs} 0'))==4 and int(cmd('debug read {VDP regs} 1'))==0x43)
...
report={'emulator':cmd('openmsx_info version').strip(),'rom':MANIFEST,'machine':'Panasonic_FS-A1ST','extension':None,'physical_hardware_tested':False,'scenario_seeding':'RAM injected only for targeted hit, collision, and distant stage/boss checks','results':results}
report['rom_slot']=cmd('machine_info external_slot slota')
