"""Deterministic V9958 SCREEN4 hardware-sprite conversion for NEON REVENANT.
...
def initializer(entries,shape):
    if not shape:
        return '{%d,%d,%d,%d}'%entries[0]
    if len(shape)==1:
        return '{'+','.join('{%d,%d,%d,%d}'%v for v in entries)+'}'
...
def main():
    ...
    for name,shape in SHAPES.items():
        ...
        for index,(source,rect) in enumerate(sources[name]):
            ...
            entries.append((offset,len(records),source.width,source.height))
    ...
    d.text((14,8),'NEON REVENANT / TURBO R ONLY / SCREEN4 HARDWARE SPRITES',fill=PALETTE[7])
