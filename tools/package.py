...
variants=[(OUT,'NEON_REVENANT-v1.2.rom',
          ...
          (OUT/'msx1/v1.3','NEON_REVENANT-MSX1-v1.3.rom',
...
for port in ['msx2','msx1']:
    sound_dir=OUT/port/('v1.3' if port=='msx1' else '')
...
msx1_dir=OUT/'msx1/v1.3'
msx1_hash=checksums[-1][0]
for name in ['layout-verification.json','reproducibility.json','giant-preservation-verification.json','archive-extraction-verification.json']:
    report=json.loads((msx1_dir/name).read_text())
    assert report['passed'],name
    if name!='giant-preservation-verification.json':
        assert report.get('rom_sha256',report.get('sha256'))==msx1_hash,name
    else:
        assert report['source_sha256']['current']==hashlib.sha256((ROOT/'msx1/src/game.c').read_bytes()).hexdigest()
...
for directory in ['src','tools','assets','turbor/src','turbor/tools','turbor/assets',
                  'msx2/src','msx2/tools','msx2/assets','msx1/src','msx1/tools','msx1/assets','msx1/tests',
                  'licenses','docs']:
...
files.add(ROOT/'msx1/GIANT_BOSS-v1.3.md')
