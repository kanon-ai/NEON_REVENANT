...
**2026-09-07追記：** 設計者から、R20／R21では分散していた設定を統合したとの補足をいただきました。これは既に記録したRTLとの比較結果に一致しています。また、EPAL有効時のSprite mode2で、各スプライトの属性テーブル+3からパレットグループを選べるようにしたはず、との説明もいただき、確認項目へ追加しました。参照したRTLでは属性の取得処理まで確認しましたが、最終的な表示色への反映には照合すべき点が残っています。本機能の動作確認済みという扱いにはせず、対象環境での表示検証を行う予定です。[属性の取得](https://github.com/hra1129/V9968_Cartridge/blob/ceeecd7e3c2d25c20045f797617af0f70ca228c1/fpga/V9968_Cartridge_TangNano20K/src/v9968/vdp_sprite_info_collect.v#L197) · [EPAL条件と色の合成](https://github.com/hra1129/V9968_Cartridge/blob/ceeecd7e3c2d25c20045f797617af0f70ca228c1/fpga/V9968_Cartridge_TangNano20K/src/v9968/vdp_sprite_makeup_pixel.v#L878)

続いて説明の全文とレジスター図も受領しました。作者は、変更点のうちモードレジスターR20／R21への注意を強調しています。図に従うR21の設定値は、V58=0のV9968側が`0x3A`、V58=1のV9958互換側が`0x3B`です。V58=0で拡張コマンドと256 KiB VRAMが有効になります。この配置を基準に、使用環境の初期化を照合します。[作者の現行資料](https://github.com/hra1129/V9968_Cartridge/tree/main/fpga/V9968_Cartridge_TangNano20K/src/v9968/manual)
...
**Update, 2026-09-07:** The designer clarified that previously separate settings were consolidated in R20/R21. This agrees with our recorded RTL comparison. The designer also said they believed palette-group selection through byte +3 of each sprite's attribute entry in Sprite mode2 had been added for use when EPAL is enabled. We have added it to our verification checklist. Attribute acquisition is present in the examined RTL, but its propagation to the final displayed color still needs clarification. We are recording this as a feature to verify on the target configuration, rather than as tested functionality. [Attribute acquisition](https://github.com/hra1129/V9968_Cartridge/blob/ceeecd7e3c2d25c20045f797617af0f70ca228c1/fpga/V9968_Cartridge_TangNano20K/src/v9968/vdp_sprite_info_collect.v#L197) · [EPAL condition and color composition](https://github.com/hra1129/V9968_Cartridge/blob/ceeecd7e3c2d25c20045f797617af0f70ca228c1/fpga/V9968_Cartridge_TangNano20K/src/v9968/vdp_sprite_makeup_pixel.v#L878)

We subsequently received the complete explanation and register diagrams. The designer emphasizes that the changes to mode registers R20/R21 need particular care. Following the diagram, R21 is written as `0x3A` for V9968 operation (V58=0), or `0x3B` for V9958 compatibility (V58=1). V58=0 enables the extended commands and 256 KiB VRAM. We will use this layout when checking initialization for the target configuration. [Designer’s current documentation](https://github.com/hra1129/V9968_Cartridge/tree/main/fpga/V9968_Cartridge_TangNano20K/src/v9968/manual)
