...
out=ROOT.parent/'outputs/msx1/v1.2';out.mkdir(parents=True,exist_ok=True)
STAGES=5
BANK_BYTES=8192
...
title_bank=6+STAGES*2
packet_first_bank=title_bank+2
packet_data={(s,p):(ROOT/'assets'/f'phase-{s}-{p:02}.bin').read_bytes()
             for s in range(STAGES) for p in range(16)}
bins=[];locations={}
for key,data in sorted(packet_data.items(),key=lambda item:(-len(item[1]),item[0])):
...
layout=['/* Generated by build.py; addresses refer to the 6000-7FFF window. */',
        ...
        f'#define WORLD_COUNT {STAGES}',f'#define TITLE_BANK {title_bank}',
        'typedef struct { unsigned char bank; unsigned int address; } WorldPacket;',
        'static const unsigned char world_banks[WORLD_COUNT] = {'+','.join(str(6+s*2) for s in range(STAGES))+'};',
...
manifest={'title':'NEON REVENANT - MSX1 PCG Drive: Breakwater to Dawn','version':'1.2','file':target.name,'mapper':'ASCII8','rom_bytes':len(rom),'allocated_bytes':used,'free_bytes':len(rom)-used,'runtime_bytes':max(mem)-0x8000+1,'runtime_address':'8000-DFFF','data_end':hex(int(area[1],16)+int(area[2],16)),'ram_required_bytes':32768,'ram_address':'8000-FFFF in one RAM slot','sprite_bytes':len(patterns),'worlds':world_stats,'video':'TMS9918A SCREEN2 256x192, 16 streamed PCG phases, two name pages, fixed palette, resident mode1 MAG2 sprites','v9990_required':False,'sha256':hashlib.sha256(rom).hexdigest(),'entry':hex(symbols['_main']),
 ...
 'packets':[{'stage':s,'phase':p,'bank':locations[s,p][0],'offset':locations[s,p][1],'bytes':len(packet_data[s,p]),'sha256':hashlib.sha256(packet_data[s,p]).hexdigest()} for s in range(STAGES) for p in range(16)]}}
