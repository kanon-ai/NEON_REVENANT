...
out=ROOT.parent/'outputs/msx1';out.mkdir(parents=True,exist_ok=True)
...
for stage in range(3):
    ...
    world_stats.append({'stage':stage+1,'bytes':len(raw),'vram_bytes':16384,'names_bytes':5120,'raw_sha256':hashlib.sha256(raw).hexdigest()})
...
for stage in range(3):
    bank_write(8+stage*2,(ROOT/'assets'/f'world-{stage}.bin').read_bytes())
    names=(ROOT/'assets'/f'names-{stage}.bin').read_bytes();assert len(names)==5120
    bank_write(14+stage,names)
bank_write(17,(ROOT/'assets/title.bin').read_bytes())
used=19*8192
target=out/'NEON_REVENANT-MSX1-v1.0.rom';target.write_bytes(rom)
(build/'symbols.json').write_text(json.dumps(symbols,indent=2))
manifest={'title':'NEON REVENANT - MSX1 Challenge','version':'1.0','file':target.name,'mapper':'ASCII8','rom_bytes':len(rom),'allocated_bytes':used,'runtime_bytes':max(mem)-0x8000+1,'runtime_address':'8000-DFFF','data_end':hex(int(area[1],16)+int(area[2],16)),'sprite_bytes':len(patterns),'worlds':world_stats,'video':'TMS9918A SCREEN2 256x192, 8 resident name phases, fixed palette, resident mode1 MAG2 sprites','v9990_required':False,'sha256':hashlib.sha256(rom).hexdigest(),'entry':hex(symbols['_main'])}
