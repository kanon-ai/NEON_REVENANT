...
u16 ui_offset,ui_score,ui_high,ui_speed;
u8 ui_valid,ui_stage,ui_shield,ui_bombs,banner_valid,banner_key;
u16 page_y(void){return ui_offset?ui_offset:((u16)draw_page)<<8;}
...
void draw_world(u8 scene){
    ...
    if(world_loaded!=scene){
        ...
        ui_valid=0;banner_valid=0;audio_mute(1);world_load(scene);world_loaded=scene;audio_mute(0);
    }
    if(mode!=PAUSED){
...
void draw_objects(void){
    ...
    for(i=0;i<BULLETS;i++)if(shots[i].active){s16 x=shots[i].x/4,y=shots[i].y/4;
        {s16 bx=x-3,by=y-4,sx=232,sy=565,w=7,h=8;
        if(bx<0){sx-=bx;w+=bx;bx=0;}if(by<0){sy-=by;h+=by;by=0;}
        if(bx+w>256)w=256-bx;if(by+h>212)h=212-by;
        if(w>0&&h>0)gfx_blit(sx,sy,bx,page_y()+by,w,h,0);}
    }
...
void hud(void){u8 i;u16 progress;
    u16 speed=520+(frame_counter&31)*3+stage*60;
    if(!ui_valid){gfx_blit(0,HUD_TOP_Y,0,1924,256,18,0);gfx_blit(0,HUD_BOTTOM_Y,0,1942,256,14,0);}
    ui_offset=1924;
    if(!ui_valid||score!=ui_score){gfx_blit(41,HUD_TOP_Y+5,41,1929,30,8,0);number(41,5,score,5);ui_score=score;}
    if(!ui_valid||highscore!=ui_high){gfx_blit(106,HUD_TOP_Y+5,106,1929,30,8,0);number(106,5,highscore,5);ui_high=highscore;}
    if(!ui_valid||stage!=ui_stage){gfx_blit(218,HUD_TOP_Y+5,218,1929,6,8,0);number(218,5,stage+1,1);ui_stage=stage;}
    ui_offset=1744;
    if(!ui_valid||shield!=ui_shield){for(i=0;i<6;i++)rect(24+i*7,203,5,5,i<shield?0xE3:0x24);ui_shield=shield;}
    if(!ui_valid||bombs!=ui_bombs){for(i=0;i<3;i++)rect(104+i*7,203,5,5,i<bombs?0x3F:0x24);ui_bombs=bombs;}
    if(!ui_valid||speed!=ui_speed){gfx_blit(168,HUD_BOTTOM_Y+4,168,1946,18,8,0);number(168,202,speed,3);ui_speed=speed;}
    ui_offset=0;ui_valid=1;
    gfx_blit(0,10116,0,page_y(),256,18,0);gfx_blit(0,10134,0,page_y()+198,256,14,0);
    if(mode==BOSS||(mode==PAUSED&&pause_previous==BOSS)){
    ...
    if(stage_banner){u8 key=stage*2+(mode==BOSS);
        if(!banner_valid||banner_key!=key){ui_offset=1918;
            rect(15,38,226,29,1);rect(15,38,226,1,0xF3);
            ...
            ui_offset=0;banner_key=key;banner_valid=1;
        }
        gfx_blit(15,10148,15,page_y()+38,226,29,0);
    }
