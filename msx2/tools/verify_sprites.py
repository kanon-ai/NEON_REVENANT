...
OUT=ROOT.parent/'outputs/msx2'
S=json.loads((ROOT/'work/build/symbols.json').read_text())
manifest=json.loads((OUT/'build-manifest.json').read_text())
results=[]
def cmd(s):
    return urllib.request.urlopen(urllib.request.Request('http://127.0.0.1:18802',data=s.encode(),method='POST'),timeout=25).read().decode().strip()
...
for crowded in [False,True]:
    ...
    atlas=(ROOT/'assets/sprites.bin').read_bytes()
    ids=block('memory',S['_sprite_ids'],n)
    ...
    check(('crowded' if crowded else 'boss')+'-sprite-table-readback',
          all(block('VRAM',(0x9800 if page else 0x1800)+(attrs[i*4+2]//4)*32,32)==atlas[ids[i]*50+2:ids[i]*50+34] for i in range(n)) and
    ...
    check(('crowded' if crowded else 'boss')+'-renderer-progress',fps>=10,
          fps=round(fps,2),max_records_per_scanline=max(counts),
          scanlines_above_hardware_limit=sum(v>8 for v in counts),pattern_uploads=read('_pattern_uploads'))
    name='crowded-sprites' if crowded else 'boss-upper-position'
    cmd(f'openmsx::internal_screenshot -raw {{{(OUT/(name+".png")).as_posix()}}}')
# A hundred frames of changing priorities, bank templates and effects exercise
# both cache pages, rather than testing only the stationary warm-cache case.
evictions=0;seen=set()
for f in range(100):
    put('_frame_counter',f,2)
    put('_keys',1 if f%3==0 else 2 if f%3==1 else 0)
    put('_boss_flash',7 if f%9==0 else 0)
    for i in range(14):cmd(f'debug write memory {S["_shots"]+i*9} {int(f%4==0)}')
    for i in range(9):
        vals=[1,(f//6+i)%3,((f+i)%6)*40+1,3,0,0,0,0,32+i*23,0,38+(i%3)*36,0]
        cmd(';'.join(f'debug write memory {S["_foes"]+i*12+j} {v}' for j,v in enumerate(vals)))
    for i in range(7):
        vals=[1,(f+i)%16,(f+i)%5,32+i*28,0,72+(i%3)*25,0]
        cmd(';'.join(f'debug write memory {S["_fx"]+i*7+j} {v}' for j,v in enumerate(vals)))
    cmd('debug cont');until(lambda:cmd('debug breaked')=='1')
    n=read('_sprite_count');page=1-read('_sprite_page');ids=block('memory',S['_sprite_ids'],n)
    attrs=block('memory',S['_attr_buffer'],n*4+(n<32));patterns=block('VRAM',0x9800 if page else 0x1800,2048)
    assert all(patterns[(attrs[i*4+2]//4)*32:(attrs[i*4+2]//4)*32+32]==atlas[ids[i]*50+2:ids[i]*50+34] for i in range(n)),('cache',f,page)
    seen.update(ids);evictions+=read('_pattern_uploads')
    assert block('VRAM',0xD800 if page else 0x5800,n*16)==block('memory',S['_color_buffer'],n*16)
    assert block('VRAM',0xDA00 if page else 0x5A00,len(attrs))==attrs
check('both-cache-pages-under-changing-priority',len(seen)>64,frames=100,distinct_templates=len(seen),uploaded_templates=evictions)
