"""Native V9938 world readback, timing and frame captures.
...
def cmd(s):
    req=urllib.request.Request('http://127.0.0.1:18802',data=s.encode(),method='POST')
...
for stage in range(3):
    ...
    fm=[]
    for i in range(4):
        fm.append(cmd('binary encode hex [debug read_block {PSG regs} 0 14]'))
        advance(.13)
    ...
    pictures=[];times=[];cameras=[];backgrounds=[];logic_frames=[]
    for i in range(64):
        ...
        backgrounds.append(read('_selected_frame'));logic_frames.append(read('_frame_counter',2))
        screenshot(p)
    ...
    check(f'stage-{stage+1}-native-moving-frames',unique>=24,unique_frames=unique,
          ...
          background_changes_per_second=round(sum(a!=b for a,b in zip(backgrounds,backgrounds[1:]))/(times[-1]-times[0]),2),
          logic_updates_per_second=round(((logic_frames[-1]-logic_frames[0])&65535)/(times[-1]-times[0]),2))
    ...
    # GIF encoders coalesce identical consecutive frames. Model that explicitly
    # and verify their full accumulated on-screen duration, not a guessed count.
    encoded=[];encoded_durations=[]
    for picture,duration in zip(enlarged,durations):
        if encoded and encoded[-1].tobytes()==picture.tobytes():encoded_durations[-1]+=duration
        else:encoded.append(picture);encoded_durations.append(duration)
    encoded[0].save(OUT/f'stage-{stage+1}-native.gif',save_all=True,
                     append_images=encoded[1:],duration=encoded_durations,loop=0,optimize=False)
    with Image.open(OUT/f'stage-{stage+1}-native.gif') as capture:
        exact=capture.n_frames==len(encoded)
        for i,source in enumerate(encoded):
            capture.seek(i)
            exact=exact and capture.convert('RGB').tobytes()==source.tobytes()
            exact=exact and capture.info['duration']==encoded_durations[i]
        check(f'stage-{stage+1}-native-gif-exact',exact,source_frames=len(pictures),encoded_frames=capture.n_frames,duration_ms=sum(encoded_durations))
