...
results=[]
def cmd(s):
    r=urllib.request.Request('http://127.0.0.1:18802',data=s.encode(),method='POST')
...
deadline=msxtime()+20
while (int(cmd('debug read {VDP regs} 1'))!=0x63 or read('_frame_counter',2)==0) and msxtime()<deadline:advance(.05)
# ROM is tested in native machine emulation, not a browser recreation.
check('target-machine',cmd('machine_info config_name').strip()=='C-BIOS_MSX2_64K',version=cmd('openmsx_info version'))
check('SCREEN4-mode',int(cmd('debug read {VDP regs} 0'))==4 and int(cmd('debug read {VDP regs} 1'))==0x63)
check('no-V9990-attached','GFX9000' not in cmd('debug list'))
check('standard-64KiB-RAM',int(cmd('debug size {Main RAM}'))==65536)
check('IM2-timer',int(cmd('reg IM'))==2)
check('PSG-only','MSX Music regs' not in cmd('debug list'))
deadline=msxtime()+20
...
up_y=read('_player_y',2);key(8,0x40,True);advance(.3);key(8,0x40,False);check('down-control',read('_player_y',2)>up_y)
...
tap(7,4)
line=bytes.fromhex(cmd(f'binary encode hex [debug read_block memory {SYM["_hud_buffer"]+32} 32]'))
caption=''.join(chr(c-192+32) for c in line)
check('NOVA-pause-caption-priority',read('_mode')==6 and read('_bomb_flash')>0 and 'PAUSED' in caption and 'NOVA' not in caption,caption=caption)
shot('pause-during-nova');tap(7,4);check('NOVA-pause-resumes',read('_mode')==1)
...
for i in range(9):cmd(f'debug write memory {SYM["_foes"]+12*i} 0')
...
report={'emulator':cmd('openmsx_info version').strip(),'rom':MANIFEST,'machine':'C-BIOS_MSX2_64K','extension':None,'physical_hardware_tested':False,'scenario_seeding':'RAM injected only for targeted hit, collision, and distant stage/boss checks','results':results}
