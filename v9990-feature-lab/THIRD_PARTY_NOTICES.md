...
## V9990 Feature Lab v0.1 への適用 / Applicability to Feature Lab

この独立した試験版にも、同梱の著作権・利用条件、免責事項、および使用する第三者コードの条件が適用されます。以下に保存した従来版の記録は、その版での検証履歴を示します。本試験版の検証範囲は README.md と outputs/ 内の検証結果を参照してください。実機では未検証で、将来の修正・サポートを保証しません。著作権帰属や許諾範囲は変更していません。

The included project rights, disclaimer and applicable third-party terms also apply to this independent feature prototype. The preserved records below describe earlier editions. See README.md and the reports under outputs/ for this build's validation scope. Physical hardware has not been tested; future fixes and support are not promised. Copyright ownership and the scope of permissions are unchanged.
