...
void update_boss(void){
    ...
    if(boss_clock%(24-stage*4)==0){
        s16 spread=(stage==2)?92:25;
...
void draw_title(void){
    ...
    rect(38,84,180,13,1);centered(88,"V9990 FEATURE LAB");
...
void draw_frame(void){
    ...
    gfx_palette_select(lab_enabled?(bomb_flash?3:(scene==2?2:((frame_counter&255)<128?0:1))):0);
    draw_world(scene);
    if(lab_enabled&&scene==1&&(mode==PLAY||(mode==PAUSED&&pause_previous==PLAY)))draw_energy_arches();
...
void main(void){
    ...
    for(;;){
        ...
        if(mode==TITLE){if(edge&16){
            new_game();
            if(keys&INPUT_UP){stage=2;mode=BOSS;boss_hp=180;boss_x=128;boss_y=82;stage_banner=90;}
        }}
