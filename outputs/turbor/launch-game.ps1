...
$romPath = Join-Path $PSScriptRoot 'NEON_REVENANT-TurboR-v1.0.rom'
...
$profilePath = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..\..\turbor\work\play-profile'))
...
$startInfo.Arguments = '-machine "{0}" -carta "{1}" -romtype ASCII8 -command "set renderer SDLGL-PP" -command "set power on" -command "set videosource MSX"' -f $Machine, $romPath
$gameProcess = [System.Diagnostics.Process]::Start($startInfo)
Write-Output "NEON REVENANT Turbo R版を起動しました (PID: $($gameProcess.Id))。標準VDPで表示します。"
