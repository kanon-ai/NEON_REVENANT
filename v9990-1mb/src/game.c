...
static const u16 stage_times[5]={900,1200,1320,1440,1560};
static const u16 boss_health[5]={70,100,140,180,240};
static const u8 spawn_period[5]={48,40,34,28,24};
static const u8 bullet_speed[5]={6,7,9,11,13};
static const u8 fire_depth[5]={146,134,122,114,106};
...
void spawn_foe(void){u8 i;u16 r=random16();for(i=0;i<ENEMIES;i++)if(!foes[i].active){
    Foe *e=&foes[i];e->active=1;e->kind=(u8)(r%(stage?3:2));e->z=6;e->age=0;
...
void spawn_wave(void){
    ...
    if(stage>=2 && (wave_number&3)==0)spawn_foe();
    if(stage==4 && (wave_number&7)==0)spawn_foe();
}
void update_objects(void){u8 i;
    for(i=0;i<ENEMIES;i++)if(foes[i].active){Foe *e=&foes[i];u8 old_z=e->z;
        ++e->age;e->z+=(e->pattern==5&&e->z>100)?1:2;
        ...
        if(old_z<fire_depth[stage]&&e->z>=fire_depth[stage]){
            ...
            if(stage>=3&&e->kind==2){fire_enemy(e->x,e->y,1);fire_enemy(e->x,e->y,2);}
        }
        if(stage>=2&&old_z<190&&e->z>=190)fire_enemy(e->x,e->y,0);
...
void update_play(void){
    ...
    if(mode==PLAY){
        ++stage_clock;
        if(stage_clock%spawn_period[stage]==0)spawn_wave();
...
void draw_world(u8 scene){
    ...
    dy=page_y()+18;
    if(lab_enabled){
        /* Ten V9990 copies make depth-dependent camera drift. The source
         * remains the full 16-phase street; no CPU pixel rewriting. */
        for(band=0;band!=10;++band){
            shift=world_camera*(band+3)/8+
                wave[(u8)(frame_counter>>2)&63]*((s16)band-3)/64;
            if(shift>0){
                gfx_blit(0,sy,shift,dy,256-shift,18,0);
                gfx_fill(0,dy,shift,18,1);
            }else if(shift<0){
                gfx_blit(-shift,sy,0,dy,256+shift,18,0);
                gfx_fill(256+shift,dy,-shift,18,1);
            }else gfx_blit(0,sy,0,dy,256,18,0);
            sy+=18;dy+=18;
        }
        return;
    }
...
void lab_line(s16 x0,s16 y0,s16 x1,s16 y1,u8 c){
    gfx_line(x0,page_y()+y0,x1,page_y()+y1,c);
}
void draw_energy_arches(void){
    u8 i,z; s16 w,t,b,l,r,k;u8 c;
    /* Project two approaching octagonal gates. LINE is the native V9990
     * command engine; these are visual scenery, not new collision walls. */
    for(i=0;i!=2;++i){
        z=(frame_counter*2+i*64)&127;
        w=12+(u16)z*110/127;t=87-(u16)z*58/127;b=99+(u16)z*94/127;
        l=128-w;r=128+w;k=w/3;c=i?0xF3:0x3F;
        lab_line(l+k,t,r-k,t,c);lab_line(r-k,t,r,t+k,c);
        lab_line(r,t+k,r,b-k,c);lab_line(r,b-k,r-k,b,c);
        lab_line(r-k,b,l+k,b,c);lab_line(l+k,b,l,b-k,c);
        lab_line(l,b-k,l,t+k,c);lab_line(l,t+k,l+k,t,c);
    }
}
...
void draw_objects(void){
    ...
    {u8 lock=0;for(i=0;i<ENEMIES;i++)if(foes[i].active&&abs16(foes[i].x-aim_x)<24&&abs16(foes[i].y-aim_y)<24)lock=1;
     ...
     if(lab_enabled)gfx_cursor(aim_x,aim_y,mode==PLAY||mode==BOSS,lock);
     else{
...
void draw_title(void){
    ...
    rect(38,84,180,13,1);centered(88,"FIVE SECTORS / ONE NIGHT");
    rect(38,105,180,51,1);rect(38,105,180,1,0xF3);
    centered(112,"ARROWS / JOYSTICK : MOVE");centered(123,"FIRE : SPACE    NOVA : X");centered(134,"ESC:PAUSE  TAB:FX ON/OFF");centered(145,"UP + FIRE : BOSS TEST");
    sprite(s,110,162);
    if(frame_counter&16)centered(188,"PRESS FIRE TO LAUNCH");
    text(8,4,"MSX TURBO R + V9990");text(190,4,"1MB ROM");
    rect(0,203,256,9,1);centered(204,"ORIGINAL CODE / ART / MUSIC  2026");
}
void draw_frame(void){
    u8 scene=stage;if(mode==TITLE)scene=0;
    gfx_cursor(aim_x,aim_y,0,0);
    /* Bank selection is applied together with page flip during VBlank. */
    /* Preserve the original three scenes in their authored blue-grey palette.
     * Dawn and short NOVA flashes alone select an alternate palette bank. */
    gfx_palette_select(lab_enabled?(bomb_flash?3:(scene==4?1:0)):0);
    draw_world(scene);
    if(lab_enabled&&scene==2&&(mode==PLAY||(mode==PAUSED&&pause_previous==PLAY)))draw_energy_arches();
...
void main(void){
    ...
    for(;;){
        ...
        if(edge&INPUT_LAB)lab_enabled^=1;
