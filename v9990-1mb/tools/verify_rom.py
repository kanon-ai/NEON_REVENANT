...
RELEASE='feature-lab-v'+VERSION
assert MANIFEST['file']==f'NEON_REVENANT-V9990-FeatureLab-v{VERSION}.rom'
ROM=(ROOT/'outputs'/MANIFEST['file']).read_bytes()
assert hashlib.sha256(ROM).hexdigest()==MANIFEST['sha256'], 'ROM differs from build manifest'
OUT=ROOT/'outputs'/RELEASE
OUT.mkdir(exist_ok=True)
results=[]
def cmd(s):
    r=urllib.request.Request('http://127.0.0.1:18890',data=s.encode(),method='POST')
...
check('target-machine',cmd('machine_info config_name').strip()=='Panasonic_FS-A1ST',version=cmd('openmsx_info version'))
check('B1-4bpp-mode',int(cmd('debug read {Sunrise GFX9000 regs} 6'))==0x81 and int(cmd('debug read {Sunrise GFX9000 regs} 13')) in (0,4,8,12))
check('R800-DRAM',int(cmd('debug read {S1990 regs} 6'))&0x60==0)
deadline=msxtime()+20
while read('_frame_counter',2)==0 and msxtime()<deadline:advance(.05)
check('startup-first-frame',read('_frame_counter',2)>0)
cmd('set pause on');write('_mode',0);write('_old_keys',0);cmd('set pause off');advance(.12);wait_world(0);shot('title')
tap(8,1);check('fire-starts-game',read('_mode')==1 and read('_shield')==6 and read('_bombs')==3)
advance(.2)
...
report={'emulator':cmd('openmsx_info version').strip(),'rom':MANIFEST,'machine':'Panasonic_FS-A1ST','extension':'gfx9000','physical_hardware_tested':False,'scenario_seeding':'RAM injected only for targeted hit, collision, and distant stage/boss checks','results':results}
report['rom_slot']=cmd('machine_info external_slot slotb')
(ROOT/'outputs'/f'verification-{RELEASE}.json').write_text(json.dumps(report,indent=2)+'\n')
