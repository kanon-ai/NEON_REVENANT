...
from neon_font import GLYPHS as NEON_GLYPHS
GLYPHS.update(NEON_GLYPHS)
...
def glyph(ch):
    ...
    for y,row in enumerate(GLYPHS.get(ch.upper(),GLYPHS['?'])):
        for x in range(5):
            if row & (1<<(4-x)):
                px[x,y]=[WHITE,WHITE,ICE,TEAL,ICE,WHITE,WHITE][y]
    return im
...
def hud_art(top):
    ...
    labels=[('SCORE',5,5),('HI',88,5),('SECTOR',174,5),('/5',230,5)] if top else [
        ('SH',5,4),('BOMB',76,4),('SPD',143,4),('KM/H',190,4)]
    for text,x,y in labels:
        draw_bitmap_text(im,text,x,y,STEEL if text!='/5' else TEAL)
    return quantize(im)
