...
for line in (build/'game.ihx').read_text().splitlines():
    ...
    if typ==0:
        for n,b in enumerate(rec[4:4+size]):
            assert 0x8000<=addr+n<0xD800,f'ROM runtime overflow: {addr+n:04X}'
            mem[addr+n]=b
...
target=out/'NEON_REVENANT-TurboR-final.rom';target.write_bytes(rom)
(build/'symbols.json').write_text(json.dumps(symbols,indent=2))
manifest={'title':'NEON REVENANT - Turbo R Edition','version':'final-2026-09-20','file':target.name,'mapper':'ASCII8','rom_bytes':len(rom),'allocated_bytes':used,'runtime_bytes':max(mem)-0x8000+1,'runtime_address':'8000-D7FF','decompression_history':'D800-DFFF','data_end':hex(int(area[1],16)+int(area[2],16)),'sprite_bytes':len(assets),'worlds':world_stats,'video':'V9958 SCREEN4 256x192, 8 resident world frames per stage, mode2 MAG2 sprites','v9990_required':False,'sha256':hashlib.sha256(rom).hexdigest(),'entry':hex(symbols['_main'])}
