/* NEON REVENANT - original 512 KiB ASCII8 game for MSX turbo R / V9958.
...
u8 pattern_buffer[1024], color_buffer[512], attr_buffer[128], hud_buffer[128];
...
s16 video_y(s16 y){return 16+(y-18)*8/9;}
void sprite(const Sprite *s,s16 x,s16 y,u8 tint){
    u8 i,j,c,early,row_y; s16 dx,dy; u16 offset=s->offset;
    const u8 *record=(const u8*)(0x6000+offset);
    u8 *patterns=pattern_buffer+((u16)sprite_count<<5);
    u8 *colors=color_buffer+((u16)sprite_count<<4);
    u8 *attributes=attr_buffer+((u16)sprite_count<<2);
    source_bank=4;source_ptr=record;
    *((volatile u8*)0x6800)=source_bank;
    y=video_y(y);
    for(i=0;i<s->count;i++){
        if(sprite_count==32){
            sprite_dropped+=s->count-i;
            source_ptr=record+(u16)(s->count-i)*SPRITE_RECORD_BYTES;
            return;
        }
        dx=x+(s8)*record++;dy=y+(s8)*record++;
        if(dx<=-32||dx>255||dy<0||dy>175){
            record+=48;++sprite_dropped;continue;
        }
        early=0;if(dx<0){dx+=32;early=128;}
        asset_copy32(record,patterns);record+=32;patterns+=32;
        row_y=(u8)dy;
        for(j=0;j<16;j++){
            c=*record++;if(tint&&c)c=tint;
            if(row_y<16||row_y>174)c=0;
            *colors++=c|early;row_y+=2;
        }
        *attributes++=(u8)(dy-1);
        *attributes++=(u8)dx;
        *attributes++=sprite_count*4;
        *attributes++=0;
        ++sprite_count;
    }
    source_ptr=record;
...
void hud(void){
    ...
    hud_text(22,2,"SPD");hud_number(26,2,520+(frame_counter&31)*3+stage*60,3);
...
void draw_objects(void){
    ...
    if(mode==BOSS||(mode==PAUSED&&pause_previous==BOSS)){
        s=&bosses[boss_sprite[stage]];sprite(s,boss_x-s->w/2,boss_y-s->h/2,boss_flash?7:0);
    }
...
void draw_frame(void){
    ...
    if(world_loaded!=stage){audio_mute(1);world_load(stage);world_loaded=stage;audio_mute(0);}
    selected_frame=(frame_counter>>1)&7;
    sprite_count=0;sprite_dropped=0;draw_objects();hud();
    if(sprite_count<32)attr_buffer[sprite_count*4]=216;
    gfx_write(sprite_page?0x9800UL:0x1800UL,pattern_buffer,(u16)sprite_count*32);
    ...
    gfx_wait_vblank();gfx_wait_vblank();
...
void main(void){
    ...
    gfx_init();title_load();world_loaded=255;sprite_page=0;world_camera=0;sound_init();audio_setup();
    for(;;){
