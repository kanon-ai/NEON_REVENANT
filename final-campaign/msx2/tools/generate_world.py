"""Convert the authored V9990 environments to resident V9938 SCREEN 4 pages.

Run ``python tools/generate_world.py`` inside the standalone MSX2 project.
...
FONT_FIRST = 192
FONT_COLOR = 0x71
from neon_font import MSX2_COLORS, pcg_font
...
def encode_frame(rgb: np.ndarray, font: bytes, hud: bool) -> tuple[bytes, np.ndarray, dict]:
    ...
    for band in range(3):
        ...
        if hud and band != 1:
            ...
            frame[0x2000+start:0x2000+start+512] = MSX2_COLORS*64
    if hud:
        target[:WORLD_TOP] = 1
    ...
    if hud:
        for band in (0, 2):
            ...
            assert frame[0x2000+start:0x2000+start+512] == MSX2_COLORS*64
...
def port_title_labels(title: Image.Image, font: bytes) -> np.ndarray:
    ...
    rgb[:12] = PALETTE[1]
    text('MSX2 / V9938 / Z80', 4, 1)
    text('512K ROM', 200, 1)
    subtitle = 'V9938 NATIVE / PSG MUSIC'
    rgb[78:89] = PALETTE[1]
...
def generate() -> dict:
    ...
    contact = Image.new('RGB', (1280, 576), PALETTE[1])
    ...
    contact.resize((2560, 1152), Image.Resampling.NEAREST).save(OUTPUT / 'contact-sheet.png')
    title_record = None
    if (SOURCE / 'title.png').exists():
        ...
        title_record = {'raw_bytes': len(data), 'sha256': sha256(data), 'source_crop': [32, 14, 288, 226],
                        'native_labels': ['MSX2 / V9938 / Z80', 'V9938 NATIVE / PSG MUSIC',
                                          'PRESS FIRE TO LAUNCH'], **stats}
        print('Title: 16384 bytes, table round trip passed', flush=True)
    manifest = {
        'format': 'V9938 SCREEN 4, 256x192, indexed 16-color palette, 2 colors per 8x1 pixels',
