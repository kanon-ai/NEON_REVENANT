...
world_streams=[];world_stats=[];banks=[];offsets=[];cursor=65536
for stage in range(5):
    raw=(ROOT/'assets/world'/f'stage-{stage+1}.bin').read_bytes();assert len(raw)==131072
    packed=compress(raw,16384);assert decompress(packed,16384,8)==raw
    banks.append(cursor//8192);offsets.append(cursor%8192)
    world_stats.append({'stage':stage+1,'raw_bytes':len(raw),'compressed_bytes':len(packed),'rom_offset':cursor,'raw_sha256':hashlib.sha256(raw).hexdigest()})
    world_streams.append(packed);cursor+=len(packed)
(ROOT/'src/world_data.h').write_text('static const unsigned char world_banks[5]={'+','.join(map(str,banks))+'};\nstatic const unsigned int world_offsets[5]={'+','.join(map(str,offsets))+'};\n')
...
rom=bytearray(boot+runtime+assets+bytes([0xFF])*(32768-len(assets)))
rom+=b''.join(world_streams)
assert len(rom)<=56*8192,'Compressed worlds overlap title at bank 56'
rom+=bytes([0xFF])*(56*8192-len(rom))+(ROOT/'assets/title.bin').read_bytes()
used=len(rom);rom+=bytes([0xFF])*(524288-len(rom));assert len(rom)==524288
target=out/'NEON_REVENANT-MSX2-final.rom';target.write_bytes(rom)
(build/'symbols.json').write_text(json.dumps(symbols,indent=2))
manifest={'title':'NEON REVENANT - MSX2 Edition','version':'final-2026-09-20','file':target.name,'mapper':'ASCII8','rom_bytes':len(rom),'allocated_bytes':used,'runtime_bytes':max(mem)-0x8000+1,'runtime_address':'8000-DFFF','data_end':hex(int(area[1],16)+int(area[2],16)),'sprite_bytes':len(assets),'worlds':world_stats,'video':'V9938 SCREEN4 256x192, 8 resident world frames per stage, mode2 MAG2 sprites','v9990_required':False,'sha256':hashlib.sha256(rom).hexdigest(),'entry':hex(symbols['_main'])}
manifest.update({'cpu':'Z80 3.58 MHz','minimum_ram_bytes':65536,'vram_bytes':131072,
                 'sound':'AY-3-8910/YM2149 PSG; no FM requirement','logic_rate_hz':30,
                 'interrupts':'IM2 F400-F500 vector, F5F5 trampoline; 60 Hz VBlank',
