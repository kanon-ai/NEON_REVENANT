...
{
    /* All calls are from the main thread. Avoid LD A,I based IFF saving:
     * an interrupt accepted on that instruction can clear P/V on a Z80. */
    __asm di __endasm;
    hw_control = value;
    hw_control = reg | 0x80;
    if(irq_ready){__asm ei __endasm;}
}
...
{
    /* VBlank IRQ enabled; 16 x 16 sprites, magnified to 32 x 32. */
...
void gfx_clock_reset(void) {gfx_last_tick=gfx_ticks;}
void gfx_clock_init(void) {
    u16 i;
    __asm di __endasm;
    /* IM2 vector F400..F500 points to a JP at F5F5. All C caller registers are saved in
     * the ISR; no BIOS or mapper access occurs. Leave F000..F2FF for stack. */
    for(i=0;i<257;i++)((u8*)0xF400)[i]=0xF5;
    *((u8*)0xF5F5)=0xC3;
    *((u16*)0xF5F6)=(u16)gfx_irq;
    gfx_ticks=0;gfx_last_tick=0;
    reg_write(15,0);(void)hw_control;
    irq_ready=1;
    __asm
        ld a,#0xF4
        ld i,a
...
void gfx_wait_vblank(void) {
    u8 previous=gfx_ticks;
    while(previous==gfx_ticks){__asm halt __endasm;}
}
u8 gfx_wait_frame(void) {
    u8 elapsed=gfx_ticks-gfx_last_tick;
    /* If preparation missed its deadline, wait for a fresh VBlank edge.
     * Testing only S2/VR is insufficient near the end of the blank interval:
     * it can end between a status read and the actual page-register writes. */
    if(elapsed>=2)gfx_wait_vblank();
    else do {elapsed=gfx_ticks-gfx_last_tick;if(elapsed<2){__asm halt __endasm;}}while(elapsed<2);
    elapsed=gfx_ticks-gfx_last_tick;
    gfx_last_tick+=elapsed&0xFE;
    elapsed>>=1;
    return elapsed>3?3:elapsed;
}
...
void gfx_init(void)
{
    ...
    reg_write(0, 0x04);      /* SCREEN 4 / GRAPHIC 3, no line interrupts. */
 ...
 * The MSX2 uses standard Z80 OTIR transfers. Native V9938 readback checks
 * validate these transfers with SCREEN 4 active; physical MSX is untested. */
